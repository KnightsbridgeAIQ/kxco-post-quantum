...
- `selftest.mjs`, which proves a responder against the pinned vectors offline
  before anything is sent to a live NIST system. Both are run on it and the two
  are directly comparable, per `conformance/results/selftest-native.json`:
  JavaScript 1512 matched, 0 mismatched, 591 refused; OpenSSL 177 matched, 0
  mismatched, 1926 refused. Each totals the same 2103 cases
